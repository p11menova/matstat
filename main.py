...
def custom_boxplot(data, title):
	"""
	Строит box-plot с учетом заданного квантиля для усов.
	:param data: список или numpy-массив с данными
	:param quantile: уровень квантиля для определения усов (по умолчанию 5%)
	"""
...
if __name__ == "__main__":
	...
	for k, v in irises_species.items():
		...
		custom_boxplot([i['sepal_length'] * i['sepal_width'] for i in v], f'площади чашелистика для вида {k}')
		# boxplot для площади лепестка
		custom_boxplot([i['petal_length'] * i['petal_width'] for i in v], f'площади лепестка для вида {k}')
